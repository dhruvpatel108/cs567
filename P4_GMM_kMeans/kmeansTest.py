...
def transform_image(image, code_vectors):
    ...
    dist = np.zeros((N*M,K))
    for k in range(K):
        dist[:,k] = np.linalg.norm(data - code_vectors[k,:], axis=1)
    
    n_cluster = np.argmin(dist, axis=1)
    assert n_cluster.shape == (N*M,), 'nearest cluster storing array should be of size N*M'
    
    new_data = np.zeros((N*M,3))
    for k in range(K):
        nk_ind = np.argwhere(n_cluster==k)
        new_data[nk_ind,:] = np.squeeze(code_vectors[k,:])
    
    new_im = np.reshape(new_data, [N,M,3])
    return new_im

...
class linear_layer:
    ...
    def forward(self, X):
        ...
        """
        forward_output = np.dot(X, self.params['W']) + np.resize(self.params['b'], [np.size(X,0), np.size(self.params['W'],1)])
    ...
    def backward(self, X, grad):
        ...
        self.gradient['b'] = np.reshape(np.sum(grad,axis=0), [1,np.size(grad,1)])

        backward_output = np.dot(self.params['W'], grad.T).T                                
...
class relu:
    ...
    def forward(self, X):
        ...
        forward_output = np.where(X<0, 0, X)
    ...
    def backward(self, X, grad):
        ...
        backward_output = np.multiply(grad, np.where(X>0, 1, 0))
